...
export default function Example2( props ) {
    ...
    const formDataChange = (e) => {
        ...
        console.log(e.target.name) // 이벤트를 발생한 html dom 의 name 속성 값 반환
        console.log(e.target.value) // 이벤트를 발생한 html dom 의 value 속성 값 반환
    ...
    const onPost = async (e) => {
        try {
            const response = await axios.post('http://localhost:8080/day07/react', formData) // state 변수를 axios 로 보내기
    ...
    // [4] 서버에 저장된 방문록 정보( 작성자/내용/비밀번호 여러개 ) 요청한다.
     ...
     // (1) 컴포넌트 마운트(생성) 될때 최초 1번 실행, 컴포넌트가 생성될때 딱 1번 onFindAll 함수가 실행된다.
    useEffect( () => { onFindAll() } , [] );
